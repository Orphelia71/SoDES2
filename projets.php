...
$totalProjects = (int)$pdo->query("SELECT COUNT(*) FROM projets")->fetchColumn();
$totalPages = max(1, (int)ceil($totalProjects / $perPage));
$page = min($page, $totalPages);
$offset = ($page - 1) * $perPage;

$stmt = $pdo->prepare("SELECT * FROM projets ORDER BY id DESC LIMIT :limit OFFSET :offset");
        ...
        <nav class="navbar">
            ...
            <div id="mobile-drawer-menu" class="mobile-navigation-drawer">
                ...
                <ul class="drawer-links-list">
                    ...
                    <li><a href="projets.php" class="drawer-link active">Projets réalisés</a></li>
    ...
    <main>
        ...
        <section class="projects-hero">
            <div class="projects-hero-container">
                <h1>Nos actions sur le terrain</h1>
            </div>
        </section>
                ...
                <?php if (!empty($projets)): ?>
                        ...
                        <?php foreach ($projets as $p): ?>
                            <article class="project-card">
                                <div class="project-image-wrapper">
                                    <img src="<?= htmlspecialchars($p['image_path']) ?>" alt="<?= htmlspecialchars($p['titre']) ?>">
                                </div>
                                <div class="project-info">
                                    <h3><?= htmlspecialchars($p['titre']) ?></h3>
                                    <?php if (!empty($p['date_projet'])): ?>
                                        <p class="project-date">Réalisé le : <?= date('d/m/Y', strtotime($p['date_projet'])) ?></p>
                                    <?php endif; ?>
                                    <a href="p-detail.php?id=<?= $p['id'] ?>" class="btn-action-yellow">Voir plus</a>
                                </div>
                            </article>
                        <?php endforeach; ?>
                    </div>
                    ...
                    <?php if ($totalPages > 1): ?>
                        <div class="pagination-wrap">
                            <?php if ($page > 1): ?>
                                <a href="projets.php?page=<?= $page - 1 ?>" class="pagination-btn">&laquo; Précédent</a>
                            <?php endif; ?>

                            <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                                <a href="projets.php?page=<?= $i ?>" class="pagination-btn <?= $i === $page ? 'active' : '' ?>"><?= $i ?></a>
                            <?php endfor; ?>

                            <?php if ($page < $totalPages): ?>
                                <a href="projets.php?page=<?= $page + 1 ?>" class="pagination-btn">Suivant &raquo;</a>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
                <?php else: ?>
                    <p class="gallery-empty">Aucun projet disponible pour le moment.</p>
                <?php endif; ?>
            </div>
        </section>
